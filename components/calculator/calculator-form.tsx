...
import { Calculator, CheckSquare, ClipboardList, RotateCcw, Send } from "lucide-react";
...
export function CalculatorForm({ catalog, value, onChange, onSubmit }: Props) {
  ...
  return (
    <section
      id="calculator"
      className="case-panel h-full w-full p-5 shadow-xl shadow-slate-900/10 md:p-6"
    >
      <div className="mb-6 flex items-center justify-between gap-3 border-b border-slate-200 pb-5">
        <div className="flex items-center gap-3">
          <div className="rounded-md bg-cyan-900 p-2 text-white">
            <Calculator className="h-5 w-5" />
          </div>
          <div>
            <p className="case-label">Заявка на расчет</p>
            <h2 className="text-xl font-semibold text-slate-950">Параметры импорта</h2>
            <p className="text-sm text-slate-500">Авто, бюджет и услуги для расчета</p>
          </div>
        </div>
        <CheckSquare className="hidden h-5 w-5 text-teal-700 sm:block" />
      </div>
      ...
      >
        <div>
          <div className="mb-3 flex items-center gap-2">
            ...
            <p className="case-label">Автомобиль из каталога</p>
          </div>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <label className="space-y-2 text-sm font-medium text-slate-700">
              ...
              <select
                ...
                value={selectedYear ?? ""}
                onChange={(event) =>
                  updateFromCatalogSelection({ year: Number(event.target.value) })
                }
            ...
            <div className="space-y-2 text-sm font-medium text-slate-700">
              ...
              <div className="min-h-[40px] rounded-md border border-slate-300 bg-white/70 px-3 py-2 text-sm text-slate-950 shadow-sm">
                {selectedVariant ? (
                  <span className="tabular font-semibold">
                    {formatMoney(value.carPrice, value.currency)}
                  </span>
                ) : (
        ...
        <div className="rounded-md border border-slate-300 bg-slate-50/70 p-4">
          <p className="case-label mb-3">Дополнительные услуги</p>
